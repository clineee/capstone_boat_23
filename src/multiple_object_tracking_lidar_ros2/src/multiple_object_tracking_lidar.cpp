...
MultipleObjectTrackingLidar::MultipleObjectTrackingLidar(const std::string &name_space,
  ...
  //std::cout << "About to setup scan callback\n";
  ...
  //std::cout << "About to setup imu callback\n";

  //Create a ROS subscriber for the IMU data
  auto imu_callback = std::bind(&MultipleObjectTrackingLidar::imu_cb, this, std::placeholders::_1);
  sub_imu = this->create_subscription<sensor_msgs::msg::Imu>("/imu/data", 1, imu_callback);
...
//IMU callback
void MultipleObjectTrackingLidar::imu_cb(const sensor_msgs::msg::Imu::ConstPtr &input) {
  //extract quaternion from the incoming IMU message
  tf2::Quaternion q(input->orientation.x, input->orientation.y, input->orientation.z, input->orientation.w);
  //convert quaternion to roll, pitch, yaw
  tf2::Matrix3x3 m(q);
  double roll, pitch, yaw;
  m.getRPY(roll, pitch, yaw);
  //update the members variables with the new roll, pitch, yaw
  this->roll = roll;
  this->pitch = pitch;
  this->yaw = yaw;
}
...
std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> MultipleObjectTrackingLidar::process_point_cloud(const sensor_msgs::msg::PointCloud2::ConstPtr &input) {
  ...
  IMU_rotation.translation().x() = 0.0;
  IMU_rotation.translation().y() = 0.0;
  IMU_rotation.translation().z() = 0.0;
  ...
  // IMU_rotation.rotate(Eigen::AngleAxisf(static_cast<float>(this->roll), Eigen::Vector3d::UnitX()) * Eigen::AngleAxisf(static_cast<float>(this->pitch), Eigen::Vector3d::UnitY()) * Eigen::AngleAxisf(static_cast<float>(this->yaw), Eigen::Vector3d::UnitZ()));
  IMU_rotation.rotate(Eigen::AngleAxisf(this->roll, Eigen::Vector3f::UnitX()) * Eigen::AngleAxisf(this->pitch, Eigen::Vector3f::UnitY()) * Eigen::AngleAxisf(this->yaw, Eigen::Vector3f::UnitZ()));

  // pcl::transformPointCloud(*input_cloud, *transformed_cloud, IMU_rotation);
  pcl::transformPointCloud(*filtered_cloud, *transformed_cloud, IMU_rotation);
  ...
  for (const auto &clusterIdx : cluster_indices) {
    ...
    cloud_cluster->points.push_back(filtered_cloud->points[pointIdx]);
	}
